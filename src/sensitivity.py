...
Run: python3 src/sensitivity.py
"""
...
def main():
    random.seed(42)
    field, pps = handicap.load()
    rows = handicap.compute_features(field, pps)
    mkt = handicap.market_probs(rows, use_live=True)
    ...
    for _ in range(N_TRIALS):
        w = perturb(handicap.WEIGHTS)
        # Rescore
        for r in rows:
            r["score_t"] = sum(w[k] * r["feats"][k] for k in w)
        scores = [r["score_t"] for r in rows]
        probs = handicap.softmax(scores, temperature=0.075)
        # Post-1 multiplier + renormalize
        adj = [p * handicap.post_multiplier(r["pp"]) for r, p in zip(rows, probs)]
        z = sum(adj)
        probs = [a / z for a in adj] if z else probs
        for r, p, mp in zip(rows, probs, mkt):
            overlay = p / mp if mp > 0 else float("inf")
            is_bet = overlay >= 1.25 and p >= 0.04
            overlay_samples[r["horse"]].append(overlay)
            if is_bet:
                bet_count[r["horse"]] += 1

    # Report — sort by bet frequency (% of trials horse was an overlay)
    print(f"\nSensitivity: {N_TRIALS} trials · weights perturbed ±{int(PERTURB_PCT*100)}% · post-1 mult applied")
